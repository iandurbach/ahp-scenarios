generate_state_evaluations <- function(Ni, J, p, skewbase, skewrange, stddevbase, stddevrange){

  # generate baseline attribute evaluations in each future
  baseA <- map(.x = as.list(1:nMainScens), .f = generate_baseline_evaluations, Ni = 9, J = 5, stddevrange = 0.04)
  ...
  skew_B <- skewbase + skewrange * runif(Ni * J * nMainScens, 0, 1)		# user-defnd skewness
  sd_B <- stddevbase + stddevrange * runif(Ni * J * nMainScens, 0, 1)  	# user-defnd stdev
  
  shape_B <- (2 / abs(skew_B)) ^ 2
  scale_B <- sd_B / sqrt(shape_B)
  
  # how many to generate in each future
  n_B <- p[rep(1:10, each = Ni * J)]
  ...
  future_index <- rep(1:nMainScens, times = Ni * J * p)
  ...
  # standardise Z within each criterion to have min U[0.1,0.2] and max U[0.8,0.9]
  # I do this so that the max ratio is 9 as per AHP.
  Zs_df <- Z_df %>% group_by(j) %>% 
    mutate(minz = runif(1,0.1,0.2),
           maxz = runif(1,0.8,0.9),
